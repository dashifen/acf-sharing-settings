...
class ACFSharingSettings extends AbstractPluginHandler
{
  ...
  protected function notifyOnMissingSettings(): void
  {
    if ($this->withACF()) {
      ...
      // the google analytics tag is optional, but all of the sharing details
      // are required if we want this to work.  therefore, we pass all of those
      // data over to the isDataMissing method.  if it returns true, then we'll
      // add a notice to the screen.
      
      if ($this->isDataMissing($this->getSharingSettings())) {
